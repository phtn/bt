...
export function EditableCell({
  ...
  return (
    <div
      className={cn(
        'w-full relative flex min-h-12 flex-col justify-between bg-white px-1 py-1 text-xs hover:bg-neutral-50 cursor-text group transition-colors',
        className,
      )}
      onClick={toggleEdit}
      {...props}>
      <span className='font-medium text-[8px] dark:text-background opacity-80 font-figtree uppercase tracking-normal select-none'>
        {label}
      </span>
      ...
      {isEditing ? (
        ...
        <div
          ...
          <span
            className={cn(
              'md:text-sm text-base text-mac-blue font-semibold',
              !value &&
