...
export function LegalDocumentPage({ document }: LegalDocumentPageProps) {
  ...
  const otherDocuments = legalDocuments.filter((doc) => doc.slug !== document.slug)
  ...
  return (
    <div className='min-h-screen bg-background'>
      ...
      <header className='sticky top-0 z-40 border-b border-border backdrop-blur supports-backdrop-filter:bg-origin/40'>
        <div className='flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8'>
          <div className='flex items-center gap-4'>
            ...
            <div>
              <h1 className='text-xl font-medium font-figtree text-foreground tracking-tighter'>{document.title}</h1>
          ...
          <div className='flex items-center space-x-2'>
            <Button
              ...
              <span className='hidden sm:inline font-figtree opacity-60'>Print</span>
            </Button>
            <button
      ...
      <div className='flex'>
        ...
        <TocDrawer isOpen={isDrawerOpen} onOpenChange={setIsDrawerOpen} document={document} />
