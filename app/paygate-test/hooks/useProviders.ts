...
import type { Provider, ProviderStatusResponse } from '../types';
  ...
  useEffect(() => {
    ...
    const fetchProviders = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await fetch('https://api.paygate.to/control/provider-status/', {
        ...
        if (!abortController.signal.aborted) {
          setProviders([
            { id: 'moonpay', provider_name: 'MoonPay', status: 'active', minimum_currency: 'USD', minimum_amount: 20 },
