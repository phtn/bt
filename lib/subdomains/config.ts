...
export const SUBDOMAIN_CONFIG = {
   ...
   */
  allowedDomains: ['localhost', 'bigticket', 'bigticket-pro.vercel.app'],
...
export function isReservedSubdomain(subdomain: string): boolean {
  return SUBDOMAIN_CONFIG.reservedSubdomains.includes(subdomain as (typeof SUBDOMAIN_CONFIG.reservedSubdomains)[number])
}

export function isAllowedDomain(domain: string): boolean {
  return SUBDOMAIN_CONFIG.allowedDomains.some((allowed) => domain === allowed || domain.endsWith(`.${allowed}`))
}
